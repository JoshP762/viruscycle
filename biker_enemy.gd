...
@export var side_offset: float = 5.0        # How far to the side it flanks
@export var commit_time: float = 3.0        # Seconds before picking a new intercept
...
@export var trail_lifetime: float = 4.0     # Seconds before segments despawn

var _player: Node3D = null
var _dead: bool = false
var _flank_side: float = 1.0               # 1.0 = right, -1.0 = left
...
var _trail_timers: Array[float] = []        # Lifetime for each segment
...
func _ready() -> void:
	...
	# Randomly pick a flank side on spawn
...
func _get_intercept_position() -> Vector3:
	...
	# Right vector relative to player's forward
...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(_player):
		# Commit timer — pick a new target every commit_time seconds
		_commit_timer -= delta
